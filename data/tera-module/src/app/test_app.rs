...
fn hello_world() {
    ...
    let response = client.get("/app").dispatch();
    ...
    //assert_eq!(html, "");
    assert!(html.contains("<title>App Rocket with Tera</title>"));
    assert!(html.contains("<h1>App Rocket with Tera</h1>"));
}

#[test]
fn page_not_found() {
    let client = Client::tracked(super::super::rocket()).unwrap();
    let response = client.get("/app/other").dispatch();
